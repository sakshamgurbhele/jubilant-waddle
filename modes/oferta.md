...
Table with:
- Detected archetype
- Domain (platform/agentic/LLMOps/ML/research/infra)
- Function (build/research/deploy/optimize)
- Seniority
...
Read `cv.md`. Create table with each JD requirement mapped to exact lines from CV.

**Adapted to archetype:**
- If LLM/GenAI → prioritize RAG, fine-tuning, LLM API proof points
- If MLOps/LLMOps → prioritize evals, observability, pipelines
- If Agentic → prioritize multi-agent, HITL, orchestration
- If AI Backend/Infra → prioritize APIs, latency, model serving, scalability
- If ML Engineer → prioritize training, fine-tuning, inference optimization
- If Research Engineer → prioritize experimentation, benchmarks, research implementation
...
2. **"Sell senior without lying" plan**: specific phrases adapted to archetype, concrete achievements to highlight, how to position experience as an advantage
...
- LLM/GenAI → emphasize production LLM features, RAG, fine-tuning delivery
- MLOps/LLMOps → emphasize metrics, evals, production hardening
- Agentic → emphasize orchestration, error handling, HITL
- AI Backend/Infra → emphasize scalability, latency, reliability
- ML Engineer → emphasize model lifecycle, training, optimization
- Research Engineer → emphasize experimentation rigor, benchmark design
